...
def get_or_create_splits(splits_dir, detections_json=None, labels_json=None):
    ...
    labels_names = set([animal for path, animal in labels_train + labels_val + labels_test])
    label_dict = {
...
def create_dataloader(images, labels, transform, batch_size=32, shuffle=False):
...
# Training loop
def train_model(model, train_dataloader, optimizer, device, num_epochs=20, checkpoint_interval=1):
    model.to(device)
...
if __name__ == "__main__":
    ...
    train_dataloader = create_dataloader(images_train, labels_train, transform, batch_size=batch_size, shuffle=True)
    val_dataloader = create_dataloader(images_val, labels_val, transform, batch_size=batch_size, shuffle=False)
    test_dataloader = create_dataloader(images_test, labels_test, transform, batch_size=batch_size, shuffle=False)

    train_model(model, train_dataloader, optimizer, device, num_epochs=num_epochs, checkpoint_interval=checkpoint_interval)
